...
void worldrendr() {
  ...
  for (long long x = -5; x <= (scrnw / 64) + 1; ++x) {
    double noiseval =
        n.GetNoise((double)blockcorner_x + x + (double)scrnoffx, (double)1024) *
            12 +
        48;
    double noiseval2 = n2.GetNoise((double)blockcorner_x + x + (double)scrnoffx,
                                   (double)1024) *
                           10 +
                       10;
    ...
    for (long long y = 0; y <= (scrnh / 64) + 1; ++y) {
      ...
      long long posy = blockcorner_y + y + scrnoffy;
        ...
        if (((absposx) < world_ref2->size()) &&
          ...
          if ((*world_ref2)[absposx][posy].generated == 0) { // world generator
            ...
            if (posy == (waterlvl)) {
              if ((*world_ref2)[absposx][posy].type == 0 &&
                ...
                if (watercheck == false) {
                  if (noiseval2 > 13 ||
                      ((*world_ref2)[absposx - 1][posy].type == 5) ||
                      ((*world_ref2)[absposx + 1][posy].type == 5)) {
                    fillwater = true;
                    watercheck = true;
                  } else {
          ...
          int c =
              (((double)((*world_ref2)[absposx][posy].light)) / 15) * 255 + 4;
          if (c > 255) {
            c = 255;
          }
